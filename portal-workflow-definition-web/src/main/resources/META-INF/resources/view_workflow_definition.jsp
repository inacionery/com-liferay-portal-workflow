...
String content = workflowDefinitionVersion.getContent();

portletDisplay.setShowBackIcon(true);
portletDisplay.setURLBack(redirect);

renderResponse.setTitle(workflowDefinitionVersion.getName());
%>
...
<div class="container-fluid-1280">
	...
	<div class="panel text-center">
		<aui:workflow-status markupView="lexicon" model="<%= WorkflowDefinitionVersion.class %>" showHelpMessage="<%= false %>" showIcon="<%= false %>" showLabel="<%= false %>" status="<%= workflowDefinitionVersion.getStatus() %>" version="<%= workflowDefinitionVersion.getVersion() %>" />
	</div>
			...
			<aui:fieldset>
				<aui:col>
					<aui:field-wrapper label="title">
						<liferay-ui:input-localized disabled="<%= true %>" name="title" xml='<%= BeanPropertiesUtil.getString(workflowDefinitionVersion, "title") %>' />
					</aui:field-wrapper>
				</aui:col>
